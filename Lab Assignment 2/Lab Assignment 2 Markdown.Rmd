...
Below we create a custom t-test function and run tests on it against a variety of data sets, comparing it to the built-in R t test. In all of the tested cases it produces comparable results to the built-in function. It is tested on various edge cases including missing values and unusual samples. It removes missing values and produces error messages for samples that have only one entry or have standard deviation of 0, similarly to how the built-in function handles these.
